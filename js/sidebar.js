window.onload = function() {
    ...
    $('A.new-bind').one('click',function () {
      elementSelected();
      chrome.devtools.panels.elements.onSelectionChanged.addListener(elementSelected);
    });

    $('A.new-unbind').click(function () {
      chrome.devtools.panels.elements.onSelectionChanged.removeListener(elementSelected);
    });
    ...
    function elementSelected() {
        ...
        startTime=d.getTime(); 
	    	$ta.val('解析中...');
        /*
        chrome.devtools.inspectedWindow.eval('(' + xxx.toString() + ')($0)', function (result,isException) {
          if (isException){
              $ta.val(result+isException+"出错").select().focus();
          }
          else{
               $ta.val(result.obj+isException+"正确").select().focus();
          }
          
        });
*/
       // $ta.val(i+":"+window.jQuery('body').html());
			chrome.devtools.inspectedWindow.eval('(' + inspect.toString() + ')($0, "' + $(this).attr('id') + '")', function (result,isException) {
        //$ta.val(result ? style_html(result, { max_char: 0 }) : 'result was null').select().focus();
        if (isException){
              $ta.val("解析遇到麻烦，请重试").select().focus();
          }
          else{
            $ta.val(result ? style_html(result, { max_char: 0 }) : 'result was null').select().focus();
            var d=new Date();
            endTime=d.getTime();
            endTime - startTime;
            $('#time').html("运行时间(ms)："+(endTime - startTime))
          }
      });
		});
	}
...
function inspect(el, func) {
  ...
  rpseudo = new RegExp( pseudos );//用于匹配非法描述匹配，eg:$('::asd:asd') 
  //正则 匹配属性
  BLOCKS_INSIDE = /[^\s{\(\)};,@%\d][^{\(\)}]*\{[^{}]*\}/g;
  /*eg
  s="{v{a:1;b:2} a{c:2}}"
  ...
  */
  ...
  var doc = el.ownerDocument,
    ...
    /*update*/
    xorSheets,
	  xorcssDoc = $('#cph_xorcss')[0].contentDocument,
	  crossstylesheets =[],
    fragCssText = '',
	...
	/*解析页面样式*/
	var xorcssDivs = xorcssDoc.getElementsByTagName("div");
	for(var x=0,l=xorcssDivs.length;x<l;x++){
		var xmlDocsheetArray = xorcssDivs[x].childNodes[0].nodeValue.replace(/\/\*.*([^\*])*[^\*]*?\*\//g,"").match(BLOCKS_INSIDE);
		var crosssheetArray = [];
        ...
        for(var i in xmlDocsheetArray){
           ...
           if(!xmlDocsheetArray.hasOwnProperty(i)){
            ...
            if(/(-moz-)|(-o-)|(-ms-)/.test(xmlDocsheetArray[i].match(/[^\s{][^{]*/)[0])){
               ...
               _xmlDocsheetstyle =  xorcssDoc.styleSheets[x].cssRules[_xmlDocsheetstyleIndex] ? xorcssDoc.styleSheets[x].cssRules[_xmlDocsheetstyleIndex].style : {}
            }
        ...
        crossstylesheets.push({"cssRules":crosssheetArray,"sheetIndex":xorcssDivs[x].id.substr(-1)})
	}
	xorSheets = crossstylesheets;
	//console.log(xorSheets)
  for (var s in docSheets) {
    if(!(docSheets.hasOwnProperty(s))){
        continue;
    }
  /*update*/
    ...
    if (thisSheet.cssRules && !isPrintOnlyStylesheet(thisSheet.media)){
        for(var i in thisSheet.cssRules){
            if(!(thisSheet.cssRules.hasOwnProperty(i))){
                continue;
            }
            try{
                var _text = thisSheet.cssRules[i].selectorText.replace(/:(hover|after|link|visited|focus|first-child|lang)/g,'')
                if(rpseudo.test(_text)){
                    continue;
                }
                else if($(thisSheet.cssRules[i].selectorText).length > 0){
                _docSheetsArray.push({"selectorText":thisSheet.cssRules[i].selectorText,"cssText":thisSheet.cssRules[i].cssText,"style":thisSheet.cssRules[i].style})
                }
            }
            catch(err) {}
            
        }
        sheets.push({"cssRules":_docSheetsArray, "sheetIndex":thisSheet.ownerNode.getAttribute('cph-ssorder'),"media":"screen"});
    }
      
  }
  for (var s in xorSheets) {
    if(!xorSheets.hasOwnProperty(s)){
        continue;
    }
    ...
    if (thisSheet.cssRules){
         for(var i in thisSheet.cssRules){
            ...
            try{
                ...
                //else if($(thisSheet.cssRules[i].selectorText).length > 0){
                   _xorSheetsArray.push({"selectorText":thisSheet.cssRules[i].selectorText,"cssText":thisSheet.cssRules[i].cssText,"style":thisSheet.cssRules[i].style})  
                }
            }
            catch(err) {}
            
        }
        sheets.push({"cssRules":_xorSheetsArray, "sheetIndex":thisSheet.sheetIndex,"media":"screen"});
    }
  }
  ...
  //return inspect_authored_trim_selectors();      
      ...
      if (rules && !isPrintOnlyStylesheet(thisSheet.media)) {
        for (var r in rules) {
          try {
            ...
            //update
            /*
  if(rpseudo.test(rules[r].selectorText)){
  ...
  */
    ...
    d=new Date();
    endTime=d.getTime();
    //console.log("本次运行时间(sec)："+(endTime-startTime)/1000);
    $el.parent().after($el).remove();
...
//      return '';
  ...
  function getComputedAuthorInheritableStyles($elm) {
      ...
      for (var s in sheets) {
        ...
        if (rules && !isPrintOnlyStylesheet(thisSheet.media)) {
          for (var r in rules) {
            ...
            //update
            if (thisRule.selectorText == undefined || rpseudo.test(thisRule.selectorText)) {
              continue;
            }

            if ($(this).is(thisRule.selectorText)) {
              thisRule.style == undefined || checkStyleForInheritableProperties(thisRule.style);
            }
          }
        }
      }
    ...
    for (var d in declaredInheritedProps) {
      if (!declaredInheritedProps.hasOwnProperty(d)) {
        continue;
      }
    ...
    for (var cssPropName in inheritedPropValues) {
      if (!inheritedPropValues.hasOwnProperty(cssPropName)) {
        continue;
      }
  ...
  function findFirstMatchGivenSelector($elm, selText) {
    var selText = selText.replace(/:(hover|after|link|visited|focus|first-child|lang)/g,'');
    if($elm.parent().find(selText).length > 0){
        return true;
    }
    else{
        return false; 
