/*重写函数*/
      var doc = $('#cph_xorcss')[0].ownerDocument,
  ...
  for(var x=0,l=xorcssDivs.length;x<l;x++){
    var xmlDocsheetArray = xorcssDivs[x].childNodes[0].nodeValue.match(BLOCKS_INSIDE);
    var crosssheetArray = [];
  ...
  /*update*/
    ...
    if (thisSheet.cssRules && !isPrintOnlyStylesheet(thisSheet.media)){
        for(var i in thisSheet.cssRules){
            ...
            try{
                if(rpseudo.test(thisSheet.cssRules[i].selectorText)){
...
window.onload = function() {
    ...
    function elementSelected() {
		$('.choice A.open').each(function () {
	    	var $ta = $(this).parent().next().find('TEXTAREA');
	    	$ta.val('inspecting...');
        alert(inspect($("#header")[0],"authored_trim_selectors"));
        $ta.val($0)
...
function inspect(el, func) {
  ...
  //inspect_authored_trim_selectors();      
      ...
      if (rules && !isPrintOnlyStylesheet(thisSheet.media)) {
        for (var r in rules) {
          try{
            ...
            if(rules[r].selectorText == undefined || rpseudo.test(rules[r].selectorText)){
              //console.log("1:"+r+rules[r].selectorText)    
              continue;
            }
            if(rules[r].selectorText.toString().indexOf(':hover') !== -1){
                    //alert(r)
                    console.dir(rulesUsed)
                    //console.log("2:"+r)
              }
              if (rules[r].selectorText  && $el.is(rules[r].selectorText)) {
                var thisRule = { cssText: rules[r].cssText, selectorText: rules[r].selectorText },
                ...
                for (var l in selectors) {
                  var thisSelector = $.trim(selectors[l]);
                  if(thisSelector == '.sm-offer-contentItem'){
                    alert(l)
                  }
...
//      return '';
  ...
  function findFirstMatchGivenSelector($elm, selText) {
    selText = selText.replace(/:(hover|after|link|visited|focus)/g,'')
